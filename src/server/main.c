...
void show_help()
{
    ...
    puts("Argumentos:");
    puts("  --port <number> Especificar el número de puerto");
    puts("  --host <name>   Especificar el hostname\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2)
    {
        for (int i = 1; i < argc; i++)
        {
            ...
            {
                hostname = argv[i + 1];
                i++; // Skip the next argument since it's the hostname
            }
            else
            {
    ...
    printf("Número de puerto configurado: %s\n", port_number);
    ...
    if ((rv = getaddrinfo(hostname, port_number, &hints, &servinfo)) != 0)
    {
